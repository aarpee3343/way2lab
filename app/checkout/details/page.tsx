...
import { User, MapPin, Plus, Check, ChevronRight, X, Home, Briefcase, Map, ArrowLeft } from 'lucide-react';
...
export default function CheckoutDetails() {
  ...
  useEffect(() => {
    ...
    if (validAddress) {
      ...
      setAddress(null);
      setShowPincodeAlert(true); // 👈 open modal instead of toast
    }
  }, [addresses, allowedPincode]);
  ...
  const selectedAddress = addresses.find(
    ...
    if (
      allowedPincode &&
      selectedAddress &&
      String(selectedAddress.pincode) !== String(allowedPincode)
    ) {
      toast.error(
        `Selected address does not match pincode ${allowedPincode}.
  Please select or add a valid address.`
      );
      return;
    }
  ...
  <div className="space-y-3">
    {addresses.map((addr: any) => {
      const isAllowed =
        !allowedPincode ||
        String(addr.pincode) === String(allowedPincode);
      ...
      return (
        <motion.div
          ...
          onClick={() => {
            if (!isAllowed) {
              toast.error(
                `You searched tests for pincode ${allowedPincode}. 
Please select or add an address with this pincode, 
or search tests again for your required address.`
              );
              return;
            }
    ...
    >
      <div className="bg-slate-100 p-1.5 rounded-full group-hover:bg-slate-200 transition-colors">
        <Plus size={18} />
      </div>
